...
makeNetworks <- function(clone_list = data,
  ...
  for(clone in 1:length(clone_list)){
    ...
    if(nrow(clone_list[[clone]]) >= 3){ # if there are enough nodes to create a tree
      ...
      g = nj(mat) # create a neighbour joining tree from the distance matrix
      brlen = compute.brlen(g) # calculate the branch lengths of the tree
      g = as.igraph(g) # turn the tree into an igraph object
      
      el = as.data.frame(get.edgelist(g)) # create an edgelist from the data
      colnames(el) = c('source', 'dest') # rename the columns
      el$weight = brlen$edge.length # assign the branch length as the edge weight
      
      el$source <- ifelse(grepl('Node', el$source),
                          paste(unique(clone_list[[clone]]$CLONE),
                                el$source, sep = '_'), el$source) # to prevent duplicate names, append the clone id to the intermediate nodes created by the nj tree
      
      el$dest <- ifelse(grepl('Node', el$dest),
                        paste(unique(clone_list[[clone]]$CLONE),
                              el$dest, sep = '_'), el$dest) # do the same for the destination nodes
      
    } else if(nrow(clone_list[[clone]]) == 2) { # if there are only two nodes in the lineage
      
      first = which.min(clone_list[[clone]][,doublet_order]) # select the source clone, based on which has the smallest distance to germline
      last = seq(1, nrow(clone_list[[clone]]))[-first] # select the destination clone (i.e., not the source clone)
      
      # use the above variables to select the clone names to input to the table
      ...
      # assign a self-linkage (will be removed later, just required for joining the tables of different sizes)
  ...
  ### join list into a dataframe + reference list
  ...
  for(i in 1:length(V(g))){
    ...
    if(grepl('Node', V(g)$name[i]) == F){ # if it's not an intermediate node created by the nj tree
      
      V(g)$germ_dist[i] <- data[grep(paste(V(g)$name[i], '$', sep = ''), data[, id_col]), germ_distance] # add in the identity to germline (can be used for colouring if required)
      V(g)$size[i] <- 6 # assign the node size as 6
      
    } else { # if it's an intermediate node from the nj tree
      
      V(g)$size[i] <- 0.001 # shrink until can't be seen
...
sequence_data <- read.csv(path_to_file, sep = file_sep) %>%
  mutate(V_FAM = gsub('^.*?(IG.V\\d).*?$', '\\1', get(v_column), ignore.case = T),
         V_IDENTITY = 1- V_IDENTITY) # include to allow correct ordering of doublets in the network.
